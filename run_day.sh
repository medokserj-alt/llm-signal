...
REPORT_ROOT="reports/day"
OUT="${REPORT_ROOT}/${TS}"
STAGING="${REPORT_ROOT}/.tmp_${TS}"
PROMPT_FILE="prompt_day.txt"
TMP_OUT="/tmp/day_${TS}.out"
MARKER="/tmp/day_${TS}.marker"
...
run_status=0
if EVENT_CALENDAR_PROFILE=day DISABLE_STATUS_SNAPSHOT=1 SIGNAL_SKIP_AIA_SEND=1 \
...
if [ "$run_status" -ne 0 ] || [ -z "${analysis_path:-}" ] || [ -z "${run_last_json:-}" ]; then
  rm -rf "$STAGING"
  rm -f "$MARKER"
  echo "❌ DAY report failed; no fresh DAY artifacts created" >&2
  exit 1
fi
